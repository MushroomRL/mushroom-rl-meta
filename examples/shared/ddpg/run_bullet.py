...
from mushroom_rl_meta.policy import SharedOrnsteinUhlenbeckPolicy
...
def experiment(idx, args):
    ...
    for i in range(len(mdp)):
        d = dataset[i::len(mdp)]
        scores[i].append(get_stats(d, gamma_eval, i, domains))
    ...
    for n_epoch in range(1, max_steps // evaluation_frequency + 1):
        ...
        for i in range(len(mdp)):
            d = dataset[i::len(mdp)]
            current_score = get_stats(d, gamma_eval, i, domains)
